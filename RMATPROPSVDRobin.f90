!****************************************************************************************************
! All-Robin-SVD R-matrix propagator for the equal-mass three-boson delta-function problem of
! Mehta, Esry & Greene, PRA 76, 022711 (2007) -- the SAME system DeltaScat.f90 solves exactly.
! EVERY box is an SVD (slow-variable-discretization) box built by RobinSVDChannelBasis.f90's
! BuildSVDBoxRobin, which gets its per-R channel data (Uad, channel-function overlap) from a
! GENUINE numerical diagonalization -- CalcBasisFuncsBP's Robin/Left=1-Right=3 boundary
! condition, fed the exact delta-function log-derivative sqrt(2*mu)*R at phi=pi/6 -- rather than
! either OneDimChannels' Poschl-Teller-well approximation (SVDChannelBasis.f90's own BuildSVDBox)
...
! Gauss-Lobatto DVR with the R=a1 node NOT a DVR node at all -- 'Radau', open/regularity boundary,
! matching AlphaFactor=0 for this problem (NOT 'LobattoDirichlet', which is only correct at
! AlphaFactor=(EffDim-1)/2=0.5); boxes 2..NumBoxes: ordinary two-sided Lobatto DVR) with the
! entire B-spline tail removed, since every box here is SVD.
!
...
PROGRAM main
  ...
  USE RobinSVDChannelBasis, ONLY: BuildSVDBoxRobin
  ...
  DO iBox = 1,NumSVDBoxes
     BLOCK
       ...
       ENDIF
       NOpR = NumChannels
       CALL BuildSVDBoxRobin(a1,a2,LThis,NumChannels,NOpL,NOpR,GType,OrderPhi,xNumPointsPhi, &
            BPDSVDBoxes(iBox),EIGSVD, &

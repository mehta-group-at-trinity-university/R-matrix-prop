...
CONTAINS
  ...
  SUBROUTINE ReadAdiabaticData(DataDir,NumChannels,NumDataPoints,muLocal,alpha,ddim, &
    ...
    !--- FitLeff.data: 2 header lines + blank, then either
    !      j, Threshold(j), residual(unused), Leff(j), FitRangeMin(j)   [5 columns, legacy]
    !    or
    !      j, Threshold(j), Leff(j), FitRangeMin(j)                     [4 columns, current]
    !    NewFitProg.f's "residual" column was always 0 (computed against an array never
    !    assigned before that point -- see its own comment) and has since been removed at
    !    the source; downstream (here) is expected to treat it as a skippable placeholder.
    !    The danger is a naive fixed-column list-directed READ silently misreading Leff as
    !    the residual or FitRangeMin when the column count doesn't match what's expected
    !    (confirmed to corrupt CalcK's outer Bessel-matching order with no read error at
    !    all -- hit twice historically, via the delta-function 5-channel dataset and via
    !    3bodydata_sech2_3boson). Guard against that by reading each line into a buffer and
    !    trying the 5-column parse first (internal READ, so a short 4-column line reliably
    !    fails with nonzero IOSTAT instead of spilling into the next record the way a direct
    !    file READ would); on failure, fall back to the 4-column parse. Only if neither
    !    succeeds is the line genuinely malformed. Both current datasets generated by
    !    NewFitProg.x (4-column) and older/hand-fixed ones (5-column, e.g. 3bodydata_delta_5ch)
    !    are supported without editing data files by hand. ---
    ...
    DO n = 1,NumChannels
       ...
       IF (ios.NE.0) THEN
          READ(fitLeffLine,*,IOSTAT=ios) j, Threshold(n), Leff(n), fitRangeMinDum
       ENDIF
       IF (ios.NE.0) THEN
          WRITE(6,*) "ReadAdiabaticData: ",trim(DataDir)//"/FitLeff.data channel ",n, &
               " matches neither the 5-column (j,Threshold,residual,Leff,FitRangeMin) nor", &
               " the 4-column (j,Threshold,Leff,FitRangeMin) format."

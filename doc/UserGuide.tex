...
\item \textbf{How is the adiabatic potential $U_n(R)$/$P_{mn}(R)$/$Q_{mn}(R)$ obtained at a
  ...
  \begin{itemize}
    \item \emph{Tabulate once, then interpolate} (\file{AdiabaticPotential.f90}): read a
      ...
      \ref{sec:gotchas}).
    \item \emph{Evaluate directly at every quadrature point needed}
...
(\file{RMATPROPHybridSVD.f90}, now superseded), ``direct+B-spline box''
(\file{RMATPROPAdiabaticDirect3Boson.f90}, \file{RMATPROPAdiabaticDirectDelta.f90}), and
``direct+SVD box''
...
\texttt{\textasciitilde/Developer/lib/}) that lets a new physical problem be added by
...
This pipeline spans three locations under \texttt{\textasciitilde/Developer/}, which is
...
\texttt{\textasciitilde/Developer/lib/} & Canonical shared numerics, used by many
  ...
  \file{AdiabaticSolverGeneric.f90}, \file{SVDBoundGeneric.f90} (bound-state spectra by Slow
  Variable Discretization), \file{FEMDVRRadial.f90} (multi-sector FEM-DVR radial grid),
  \file{ScatteringModules.f90}, \file{units.f90}; \file{AtomIonAsymptotics.f90} (closed-form
  large-$R$ $U$/$P$/$Q$ for atom-ion in a trap, added 2026-09-23 --- four local copies still
  to be repointed); special functions \file{hyp.f} (hypergeometric, Forrey 1997); random
  matrices \file{GOE.f}, \file{testGOE.f} with the RANLUX generators \file{ranlux.f},
  \file{rgnf\_lux.f}; and shared quadrature data \file{Legendre.dat}, \file{GaussLobatto.dat}. \\
\texttt{\textasciitilde/Developer/interpolation/} & Canonical B-spline interpolation
...
be kept in sync by hand.} If you fix a bug in one (as was done for \sub{GridMakerHHL}'s
...
\item \textbf{\sub{GridMaker} name collisions.} At least three incompatible subroutines named
  ...
  This arose when adding a generic-grid \sub{GridMaker} call to ASB's
...
\item \textbf{\file{GaussLobatto.dat}/Gauss-Radau tables.} \sub{GetGaussLobattoFactors} (used by
  ...
  simultaneously, which \emph{no pair} in the shipped table satisfies -- two extra blocks
  (N=20,21) were computed and appended with a short Python/\texttt{numpy.polynomial} script,
  matching the file's existing tab-separated format exactly. If you pick a new $L$, check
...
\subsection{Layer 0 -- Shared numerics (\texttt{\textasciitilde/Developer/lib/})}
...
\item[\file{AdiabaticPotential.f90} -- tabulate + interpolate] \hfill \\
  ...
  avoid Runge ringing) via \texttt{\textasciitilde/Developer/interpolation}'s
...
\file{RMATPROPAdiabaticDirect3Boson.x} & B-spline & Direct
  (\file{SechAdiabaticPotentialDirect.f90}) & Same structure as
...
\file{RMATPROPHybridSVDGeneric.f90} (module \texttt{RMATPROPHybridSVDGenericMod}, subroutine
  \sub{RunHybridSVDGeneric}) & SVD $+$ B-spline tail & Pluggable (\texttt{MyPotential}/\texttt{MyGridMaker}/
  ...
  \texttt{Box1GridType} (\texttt{'Radau'} or \texttt{'LobattoDirichlet'} --
  ...
  \textbf{Implements the tail itself}: \texttt{haveTail\,=\,(NumBoxesIn>NumSVDBoxes)}, after which
  boxes \texttt{NumSVDBoxes+1..NumBoxesIn} are ordinary B-spline boxes off the caller's
  \texttt{UInterp}/\texttt{PInterp}/\texttt{QInterp}. Callers pass \texttt{NumBoxesIn} equal to
  \texttt{NumSVDBoxes} to get pure SVD. \\
\file{RMATPROPHybridSVDGenericDelta.f90} & SVD $+$ B-spline tail & \file{DeltaFunctionPlugins.f90} & Thin driver
  ...
  here was wrong). Off the exact Mehta-Shepard result by $\pi/2$ at \emph{every} $q$ (mod-$\pi$
  reduced; the raw unwrapped difference reaches $8.8$\,rad) -- see the benchmark table in
  Section~\ref{sec:validation}. The generic
  ...
  header banner. Use \file{RMATPROPSVDRobin.x} or \file{RMATPROPAdiabaticDirectDelta.x}.
  (Tail via an optional \texttt{.inp} trailer; absent means pure SVD. The defect above afflicts the
  SVD boxes, so a tail does not rescue it.) \\
\file{RMATPROPHybridSVDGenericSech.f90} & SVD $+$ B-spline tail & \file{SechFunctionPlugins.f90} &
  Thin driver for the (2-fermion+1-distinguishable) sech$^2$ case, domain $[0,\pi/2]$. Tail via an
  optional \texttt{.inp} trailer (\texttt{NumBoxesTotal} $+$
  \texttt{OrderTail}/\texttt{xNumPointsTail}/\texttt{LegPointsTail}); an \texttt{.inp} without it
  runs pure SVD. \\
\file{RMATPROPHybridSVDGenericSech3Boson.f90} & SVD $+$ B-spline tail & \file{SechFunctionPlugins.f90} & Thin
...
\paragraph{Naming convention: \texttt{Hybrid} means the driver can switch box type mid-chain.}
A driver is named \file{RMATPROPHybridSVD*} only if it can propagate SVD boxes \emph{and} ordinary
adiabatic (B-spline) boxes in one run -- i.e. it accepts a total \texttt{NumBoxes} exceeding
\texttt{NumSVDBoxes} and builds the remainder from tabulated \texttt{Uad}/\texttt{Pmat}/\texttt{Qmat}.
Drivers that only ever build SVD boxes are named \file{RMATPROPSVD*}
(\file{RMATPROPSVDAnalytic.x}, \file{RMATPROPSVDRobin.x}). Those two have no tail machinery
\emph{at all}: no \sub{ReadAdiabaticData}, no \texttt{BPData}, no
\texttt{NumSVDBoxes+1..NumBoxes} loop anywhere, and \texttt{NumBoxes} is \emph{assigned} from
\texttt{NumSVDBoxes} rather than read from the \texttt{.inp}, so no input can reach a tail. Their
\texttt{NumBoxes} is chain bookkeeping only -- box-edge spacing denominator, \texttt{Boxes(:)}
allocation, and \sub{CalcK}'s outermost-box argument -- not a switch. \file{RunHybridAtomIon.x}
in \repo{VeffAtomIon1D} also satisfies the rule (SVD boxes to $x_{\text{switch}}$, then one ordinary
B-spline box built by \sub{SetAtomIonAsymptoticPotential}/\sub{CalcGamLam}).
...
\item[\file{DriveAdiabaticSolver1D.f90}] \texttt{PROGRAM}, reads its parameter file from stdin,
  ...
  now built via a locally-added \sub{GridMakerScale(R,RSteps,RMin,RMax,"quadratic")} (formerly
  uniform, which left almost no resolution near $R=0$; see
...
This is the single most confusing piece of the whole pipeline, and the source of most of the
debugging effort it has cost. Two \emph{independent} choices interact here and are easy to
conflate:
...
B-spline \texttt{Left=2} + \sub{PartitionAndEliminate} & Either $\alpha$ convention & Both
  ...
  both actually use}; their box geometry and grid construction have been confirmed
  identical. \\
...
\texttt{BuildBox1CombinedBasis} (\texttt{Left=3}, Robin, legacy) & Neither, in general & Computes
  ...
  (special-function underflow at small argument). The failure mode is astronomic, nonsensical
  $K$-matrix values; note \file{RMATPROPAdiabatic.f90} does not call this path any more. \textbf{Not needed for new work}; understand it only
...
This section walks through the steps followed in building the sech$^2$
...
\item Decide whether you need the \emph{full} angular domain or can exploit permutation/parity
  symmetry to restrict to a smaller fundamental domain (as is done for the
  3-identical-boson sech$^2$ problem, reduced from $[0,\pi/2]$ to $[0,\pi/6]$, mirroring what the
  ...
  the edge-refining variant for the reduced domain) are templates
  ...
  will hit the degenerate-segment bug documented in} Section~\ref{sec:gotchas} --
...
\item A \sub{RobinCoeffProc}: return the sentinel \texttt{1d20} for both outputs unless you
  ...
  example).
\end{enumerate}
\textbf{Validate the plugin in isolation before trusting it in the full pipeline.} The
most productive debugging technique here is writing small, throwaway standalone comparator programs
...
\item \textbf{Angular resolution} (\texttt{xNumPoints}/\texttt{LegPoints} for the angular
  ...
  comparing against. There is no measurable difference between $\phi$NumPoints=80 and 200 for a
  single-point curve check, but 200 (matching the original ASB tabulation) is what to use for
  anything meant to be authoritative.
...
\item \textbf{Box 1's own internal knot grid}: already quadratic-clustered toward \texttt{xl} via
  ...
  caused by another. The ``try a quadratic grid'' fix for the near-origin interpolation problem
  specifically means the \emph{tabulation} grid, leaving the other two unchanged.
\end{itemize}
...
$\sim$0.05\,s at 200-point angular resolution (measured with \file{SechCurvesSVD.x}: 400
...
Two implementation bugs are worth remembering if you write another
...
short blocks with zero -- \file{AdiabaticSolverGeneric.f90} always zero-pads and writes the full
block, since this was the first code path in the workspace to round-trip that data through a
...
\item \textbf{Literal $R=0$ is unusable for direct potential evaluation.} The hyperspherical
  ...
  \texttt{info=-9999}), and that single failure then
...
\item \textbf{The interpolant's own domain floor is a hard constraint, not just an accuracy
  ...
  over \texttt{Rdata(1:NumDataPoints)} (it formerly used
...
\item \textbf{\sub{GridMakerHHL}'s 4-segment refinement scheme assumes the interior coalescence
  ...
  with no direct clue pointing at the grid maker. Fixed in both repos' copies of
...
and literature benchmarks. Techniques that have proved effective, roughly in order of
...
\item \textbf{Standalone A/B comparators.} A small throwaway program calling two code paths (e.g.
  ...
  is how the dropped-square-root \texttt{RPair} typo was found -- it was invisible in
...
\item \textbf{Keep a standing cross-driver benchmark table.} Several drivers solve the
  \emph{same} physics by structurally different routes, so running them at identical settings and
  tabulating the result catches a regression in any one of them. For the equal-mass 3-boson
  delta-function problem (exact atom-dimer $K$-matrix from Mehta-Shepard), at
  \texttt{NumBoxes}=20, $[0,100]$, 5 channels, 150 quadratic energies:

  \begin{center}
  \let\texttt\ttb
  \begin{tabular}{@{}lrrr@{}}
  \toprule
  \textbf{Driver} & $q>0.1$ & $q>0.3$ & $q\tan\delta|_{q\to0}$ \\
  \midrule
  \file{RMATPROPAdiabatic.x} (tabulated)   & $7.60\times10^{-3}$ & $7.60\times10^{-3}$ & $+0.00021$ \\
  \file{RMATPROPSVDAnalytic.x}       & $8.22\times10^{-3}$ & $8.22\times10^{-3}$ & $-0.00000$ \\
  \file{RMATPROPSVDRobin.x}          & $8.24\times10^{-3}$ & $8.24\times10^{-3}$ & $-0.00004$ \\
  \file{RMATPROPAdiabaticDirectDelta.x}    & $7.91\times10^{-3}$ & $7.91\times10^{-3}$ & $+0.00064$ \\
  \file{RMATPROPHybridSVDGenericDelta.x}   & $1.57$              & $1.57$              & $+0.00989$ \\
  \bottomrule
  \end{tabular}
  \end{center}

  Columns are $\max|\delta-\delta_{\text{exact}}|$ reduced mod $\pi$. \textbf{Compare mod $\pi$, not
  raw:} \texttt{delta} is written unwrapped with no anchor, so each driver's branch differs by an
  arbitrary $n\pi$ (Section~\ref{sec:leffconv}) -- a raw diff is dominated by that offset and says
  nothing. The exact $q\tan\delta\to0$ as $q\to0$, so the last column is a convention-free
  near-threshold check. The first four agree at ${\sim}8\times10^{-3}$\,rad, which is the ordinary
  5-channel truncation floor, \emph{not} a numerical artifact -- four independent routes
  (interpolated tabulation, closed-form SVD, numerically-diagonalized SVD, and direct B-spline)
  landing together is what establishes that. The fifth sits at $\pi/2$, i.e.\ no agreement at any
  $q$ (gotcha~\ref{got:sout}).

  \textbf{Regenerate with:} run each driver at the settings above, then \file{MehtaShepardExact.x}
  (which reads \texttt{PhaseShift.dat} and writes \texttt{MehtaShepardExact.dat} with numerical and
  exact side by side). \textbf{Delete \texttt{PhaseShift.dat} before each run and check it
  reappeared} -- every driver here writes that same filename, so a failed run silently leaves the
  previous driver's output in place and the comparison then looks plausible but is meaningless.
